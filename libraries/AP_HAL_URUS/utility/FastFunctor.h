...
public:
	...
	typedef FastFunctor0 type;

	// Construction and comparison functions
	FastFunctor0() { clear(); }
	FastFunctor0(const FastFunctor0 &x) {
		m_Closure.CopyFrom(this, x.m_Closure); }
	void operator = (const FastFunctor0 &x)  {
		m_Closure.CopyFrom(this, x.m_Closure); }
	bool operator ==(const FastFunctor0 &x) const {
	...
	FastFunctor0(Y *pthis, DesiredRetType (X::* function_to_bind)(Params...) ) {
	...
	template < class X, class Y >
	FastFunctor0(const Y *pthis, DesiredRetType (X::* function_to_bind)(Params...) const) {
...
// FastFunctor< R ( Params... ) >
// instead of 
// FastFunctor0 < Params..., R >
template<typename R, class... Params>
class FastFunctor< R ( Params... ) >
  // Inherit from FastFunctor0 so that it can be treated just like a FastFunctor0
  : public FastFunctor0 < R, Params... >
{
public:
  // Make using the base type a bit easier via typedef.
  typedef FastFunctor0 < R , Params... > BaseType;
  ...
  FastFunctor(R (*function_to_bind)( Params... ))
...
FastFunctor0<FASTDLGT_RETTYPE, Params...> MakeDelegate(Y* x, RetType (X::*func)(Params...)) { 
	return FastFunctor0<FASTDLGT_RETTYPE, Params...>(x, func);
}

template <class X, class Y, class RetType, class... Params>
FastFunctor0<FASTDLGT_RETTYPE, Params...> MakeDelegate(Y* x, RetType (X::*func)(Params...) const) { 
	return FastFunctor0<FASTDLGT_RETTYPE, Params...>(x, func);
}
