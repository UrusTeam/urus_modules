...
uint8_t AP_InertialSensor::register_gyro(void)
{
    if (_gyro_count == INS_MAX_INSTANCES) {
        hal.console->printf_PS(PSTR("Too many gyros\n"));
    }
    return _gyro_count++;
}
...
uint8_t AP_InertialSensor::register_accel(void)
{
    if (_accel_count == INS_MAX_INSTANCES) {
        hal.console->printf_PS(PSTR("Too many accels\n"));
    }
    return _accel_count++;
}

void
AP_InertialSensor::init(uint16_t sample_rate, Start_style style)
{
    // remember the sample rate
    _sample_rate = (Sample_rate)sample_rate;
...
{
    if (!backend) {
        hal.console->printf_PS(PSTR("INS No Backend\n"));
        return;
    }

    if (_backend_count == INS_MAX_BACKENDS)
        hal.console->printf_PS(PSTR("Too many INS backends\n"));
...
AP_InertialSensor::_detect_backends(void)
{
    ...
    _add_backend(AP_InertialSensor_MPU6000::probe(*this, hal.i2c_mgr->get_device(HAL_INS_MPU60XX_I2C_BUS, HAL_INS_MPU60x0_I2C_ADDR)));
...
#elif HAL_INS_DEFAULT == HAL_INS_ITG32XX
    AP_InertialSensor_Backend *backend = new AP_InertialSensor_ITG3200(*this,
            std::move(hal.i2c_mgr->get_device(HAL_INS_ITG32XX_GYRO_I2C_BUS, HAL_INS_ITG32XX_I2C_GYRO_ADDR)),
            std::move(hal.i2c_mgr->get_device(HAL_INS_ITG32XX_ACCEL_I2C_BUS, HAL_INS_ITG32XX_I2C_ACCEL_ADDR)));
    ...
    if (_backend_count == 0 ||
        _gyro_count == 0 ||
        _accel_count == 0) {
        hal.console->printf_PS(PSTR("No INS backends available\n"));
    }
    ...
    */
        ...
        while (num_samples < 400/update_dt_milliseconds) {
            ...
            for (uint8_t k=0; k<num_accels; k++) {
                ...
                if (!get_accel_health(k)) {
                    if (interact) {
                        interact->printf_PS(PSTR("accel[%u] not healthy"), (unsigned)k);
                    } else {
                        hal.console->printf_PS(PSTR("accel[%u] not healthy\n"), (unsigned)k);
                    }
                    goto failed;
                }
            }
    ...
    for (uint8_t k=0; k<num_accels; k++) {
        if (!_check_sample_range(samples[k], saved_orientation, interact)) {
            if (interact) {
                interact->printf_PS(PSTR("Insufficient accel range"));
            } else {
                hal.console->printf_PS(PSTR("Insufficient accel range\n"));
            }
            continue;
        }
        ...
        if (interact) {
            interact->printf_PS(PSTR("Offsets[%u]: %.2f %.2f %.2f\n"),
                                (unsigned)k,
                               (double)new_offsets[k].x, (double)new_offsets[k].y, (double)new_offsets[k].z);
            interact->printf_PS(PSTR("Scaling[%u]: %.2f %.2f %.2f\n"),
                               (unsigned)k,
                               (double)new_scaling[k].x, (double)new_scaling[k].y, (double)new_scaling[k].z);
        } else {
            hal.console->printf_PS(PSTR("Offsets[%u]: %.2f %.2f %.2f\n"),
                                (unsigned)k,
                               (double)new_offsets[k].x, (double)new_offsets[k].y, (double)new_offsets[k].z);
            hal.console->printf_PS(PSTR("Scaling[%u]: %.2f %.2f %.2f\n"),
                               (unsigned)k,
                               (double)new_scaling[k].x, (double)new_scaling[k].y, (double)new_scaling[k].z);
        }
        if (success) num_ok++;
    ...
    if (num_ok == num_accels) {
        if (interact) {
            interact->printf_PS(PSTR("Calibration successful\n"));
        } else {
            hal.console->printf_PS(PSTR("Calibration successful\n"));
        }
...
failed:
    if (interact) {
        interact->printf_PS(PSTR("Calibration FAILED\n"));
    } else {
        hal.console->printf_PS(PSTR("Calibration FAILED\n"));
    }
...
AP_InertialSensor::_init_gyro()
{
    ...
    */
    ...
    for (uint8_t k=0; k<num_gyros; k++) {
        if (!converged[k]) {
            hal.console->printf_PS(PSTR("gyro[%u] did not converge: diff=%f dps\n"),
                                            ...
                                            AP_InertialSensor_UserInteract* interact)
{
    ...
    if (interact) {
        interact->printf_PS(PSTR("AccelRange: %.1f %.1f %.1f"),
                (double)range.x, (double)range.y, (double)range.z);
    } else {
        hal.console->printf_PS(PSTR("AccelRange: %.1f %.1f %.1f\n"),
                (double)range.x, (double)range.y, (double)range.z);
    }
