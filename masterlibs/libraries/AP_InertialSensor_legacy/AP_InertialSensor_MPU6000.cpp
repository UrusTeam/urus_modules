...
AP_MPU6000_BusDriver_SPI::AP_MPU6000_BusDriver_SPI(AP_HAL::OwnPtr<AP_HAL::SPIDevice> dev) :
    _spi(std::move(dev)),
    _error_count(0)
{
}
...
void AP_MPU6000_BusDriver_SPI::set_bus_speed(AP_HAL::SPIDevice::Speed speed)
{
    _spi->set_speed(speed);
}
                                         ...
                                         uint8_t &n_samples)
{
    ...
    _spi->transfer((const uint8_t *)&tx, sizeof(tx), (uint8_t *)&rx, sizeof(rx));
    ...
    */
    ...
    if (i == 14) {
        ...
        if (++_error_count > 4) {
            set_bus_speed(AP_HAL::SPIDevice::Speed::SPEED_LOW);
        }
    }
...
AP_MPU6000_BusDriver_I2C::AP_MPU6000_BusDriver_I2C(AP_HAL::OwnPtr<AP_HAL::I2CDevice> dev) :
    _i2c(std::move(dev)),
    _i2c_sem(NULL)
{}
...
{
    _i2c->read_registers(reg, val, 1);
}

void AP_MPU6000_BusDriver_I2C::write8(uint8_t reg, uint8_t val)
{
    _i2c->write_register(reg, val);
}

void AP_MPU6000_BusDriver_I2C::set_bus_speed(AP_HAL::I2CDevice::Speed speed)
{
    _i2c->set_speed(speed);
}
                                          ...
                                          uint8_t &n_samples)
{
    ...
    //uint8_t ret = 0;
/*
    ret = _i2c->read_registers(MPUREG_FIFO_COUNTH, _rx, 2);
    if(ret != 0) {
        hal.console->printf_PS(PSTR("MPU6000: error in i2c read\n"));
        n_samples = 0;
        return;
    }
*/
    _i2c->read_registers(MPUREG_FIFO_COUNTH, _rx, 2);
    bytes_read = uint16_val(_rx, 0);

    n_samples = bytes_read / MPU6000_SAMPLE_SIZE;

    if(n_samples > 3) {
        hal.console->printf_PS(PSTR("bytes_read = %d, n_samples %d > 3, dropping samples\n"),
        ...
        _i2c->read_registers(MPUREG_FIFO_R_W, _rx, n_samples * MPU6000_SAMPLE_SIZE);
    }
/*
    if(ret != 0) {
        hal.console->printf_PS(PSTR("MPU6000: error in i2c read %d bytes\n"),
                                                               ...
                                                               enum Rotation rotation)
{
    ...
    AP_MPU6000_BusDriver *bus = new AP_MPU6000_BusDriver_I2C(std::move(dev));
    if (!bus)
        return nullptr;
    return _detect(imu, bus);
}
...
/*
    ...
    sensor = new AP_InertialSensor_MPU6000(imu, std::move(dev));
...
*/
    dev->set_read_flag(0x80);
    AP_MPU6000_BusDriver *bus = new AP_MPU6000_BusDriver_SPI(std::move(dev));
    if (!bus)
        return nullptr;
    return _detect(imu, bus);
}
...
bool AP_InertialSensor_MPU6000::update( void )
{
    ...
    if (_last_accel_filter_hz != _accel_filter_cutoff()) {
        if (_bus_sem->take(10)) {
            _bus->set_bus_speed(AP_HAL::Device::SPEED_LOW);
            _set_filter_register(_accel_filter_cutoff());
            _bus->set_bus_speed(AP_HAL::Device::SPEED_HIGH);
            _bus_sem->give();
            _last_accel_filter_hz = _accel_filter_cutoff();
        }
    }
...
void AP_InertialSensor_MPU6000::_register_write_check(uint8_t reg, uint8_t val)
{
    ...
    if (readed != val){
        hal.console->printf_PS(PSTR("Values doesn't match; written: %02x; read: %02x "), val, readed);
    }
#if MPU6000_DEBUG
...
bool AP_InertialSensor_MPU6000::_hardware_init(void)
{
    ...
    if (!_bus_sem->take(100)) {
        hal.console->printf_PS(PSTR("MPU6000: Unable to get semaphore\n"));
    }
