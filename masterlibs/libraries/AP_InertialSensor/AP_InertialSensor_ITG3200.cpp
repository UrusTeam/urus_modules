...
{
    _init_sensor(RATE_100HZ);
    _gyro_instance = _imu.register_gyro(100, 0);
    _accel_instance = _imu.register_accel(100, 1);
}
...
bool AP_InertialSensor_ITG3200::update( void )
{
    ...
    _imu.set_gyro(_gyro_instance, Vector3f(_gyro_data_sign[0] * sum[_gyro_data_index[0]],
    ...
    _imu.set_accel(_accel_instance, Vector3f(_accel_data_sign[0] * sum[_accel_data_index[0]],
    ...
    _rotate_and_correct_accel(_accel_instance, _accel);
    _rotate_and_correct_gyro(_gyro_instance, _vgyro);

    //update_accel(_accel_instance);
    //update_gyro(_gyro_instance);

    _notify_new_accel_raw_sample(_accel_instance, _accel, 0, false);
    _notify_new_gyro_raw_sample(_gyro_instance, _vgyro);

    //_publish_gyro(_gyro_instance, _vgyro);
    //_publish_accel(_accel_instance, _accel);
