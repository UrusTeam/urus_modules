...
const u8 STRING_PRODUCT[] PROGMEM = TOSTRING(USB_PRODUCT);
...
const u8 STRING_MANUFACTURER[] PROGMEM = TOSTRING(USB_MANUFACTURER);
...
bool SendDescriptor(USBSetup& setup)
{
	...
	else if (USB_STRING_DESCRIPTOR_TYPE == t)
	{
		if (setup.wValueL == 0) {
			...
			return USB_SendStringDescriptor(STRING_PRODUCT, strlen(TOSTRING(USB_PRODUCT)), TRANSFER_PGM);
		}
		else if (setup.wValueL == IMANUFACTURER) {
			return USB_SendStringDescriptor(STRING_MANUFACTURER, strlen(TOSTRING(USB_MANUFACTURER)), TRANSFER_PGM);
		}
...
void CLCoreUrusUsbDevice_Avr::attach()
{
	_usbConfiguration = 0;
	_usbCurrentStatus = 0;
	_usbSuspendState = 0;
